...
function setFields(id, name, HideField, textField, Resulbox){
	...
	function addStaff(id, name, HideField, textField, Resulbox){
		...
		html+=name+'<span onclick="removeStaff('+id+')">X</span><input type="hidden" name="otherStaf[]" value="'+id+'">';
	...
	function addFieldIndividual(){
		...
		$.ajax({
			...
			$('.only-individual').removeClass('hidden');
	...
	function RemoveFieldIndividual(){
		...
		$('.only-individual').addClass('hidden');
	...
	function thisSelectNew(valForHidden, selectedString, idHidden, button ){

		$('#'+idHidden).val(valForHidden);

		//$('#'+button).html(selectedString.substring(0, 14));

		$('#ContactResult_VU').removeClass('hidden').append('<div class="container-user-new"><button type="button" class="close" aria-hidden="true">&times;</button><p class="text-center">'+selectedString.substring(0, 14)+'</p></div>');
	...
	$( document ).ready(function() {
		...
		$("#close_matter").click(function() {
			...
			var url = base_app + 'Task/getListToMarkCompleted';
			...
			$.ajax({
				...
				$("#closeTaskModal .modal-body").html('');

				if(data.trim() == 'no data'){

					$("#closeTaskModal .modal-body").append('<p>No ha seleccionada ninguna tarea aún</p>');

				}else{

					var inputTask = $("input[type='checkbox']:checked");

					for (var i = 0; i <  inputTask.length ; i++) {
						$("#closeTaskModal .modal-body").append('<pre>'+ JSON.parse(data)[$(inputTask[i]).val()] +'</pre>');

					}
				}

			}).fail(function (status, error, throw_response){
				console.log(status + "  --  "+ error + "  --  " + throw_response);
			});
