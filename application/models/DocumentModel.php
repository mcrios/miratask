...
class DocumentModel extends CI_Model {
	...
	{
		$sql = "SELECT * FROM ml_do_folders ORDER BY Name";
    ...
    public function deleteDocument($Id, $actualDir){

		$this->db->trans_begin();    
		
		$this->db->select();
		
        $this->db->from('ml_do_atach');
        
		$this->db->where( array('Document' => $Id ));

        
		$objetosAsociados = $this->db->get()->result();
        var_dump($objetosAsociados);


		foreach($objetosAsociados as $row){
			
			$this->db->where(array( 'Id' =>  $row->Id ));
			$this->db->delete("ml_do_atach");
		...
		$this->db->select();
		$this->db->from('ml_do_documents');
		$documento = $this->db->get()->row();
		...
		if ($this->db->trans_status() === FALSE){      
			...
			chdir($actualDir . "\\docsxup\\2019");

			$datos = shell_exec("dir");
	
			$nombreDocumento = $documento->FileName;
	
			if(!strpos($datos, $nombreDocumento)){
				chdir($actualDir . "\\docsxup\\2018");
			}
	
			unlink(trim($nombreDocumento));		
	
	
			$this->db->where( array('Id' => $Id) );
			$this->db->delete("ml_do_documents");
