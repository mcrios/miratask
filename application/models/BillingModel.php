...
class BillingModel extends CI_Model {
	...
	function update_invoice($invoice,$amount)
	{
		$this->db->set('InvoiceAmount', $amount);
	...
	function get_matter_name($id){
		$sql = "SELECT b.* 
		FROM ml_ma_matters a 
		INNER JOIN ml_co_contact b
		ON a.`Client` = b.Id
		WHERE a.Id = ?";
	...
	function getTotalInvoices($filtro){
		$sql = "SELECT count(a.Id) cant FROM ml_bi_invoices a WHERE a.Id is not null  AND Status='Draft'".$filtro."  ";
		//echo $sql;
	...
	function invoices($page,$per_page,$filtro){
		...
		(CURRENT_TIMESTAMP < a.dueDate) AS estadoVencimiento, CURRENT_TIMESTAMP as hoy
	...
	function invoicesDue($page,$per_page,$filtro){
		$sql="SELECT a.*, 
		CASE 
		WHEN a.TypeObject='contact'  
		THEN (SELECT CONCAT(FirstName, ' ', LastName) AS ClientName FROM ml_co_contact WHERE Id=a.Object)
		...
		(CURRENT_TIMESTAMP < a.dueDate) AS estadoVencimiento, CURRENT_TIMESTAMP as hoy 
		FROM ml_bi_invoices a 
		WHERE  a.Id is not null ".$filtro." AND Status='Draft' ORDER BY a.Id DESC limit ".$page.",".$per_page." ";
	...
	function getEstadistic(){
		...
		(SELECT SUM(InvoiceAmount) FROM ml_bi_invoices WHERE Status = 'Draft' AND DueDate >= '$today') AS Outstanding 
	...
	function getEstadisticOverdue(){
		...
		$stringQuery = " SELECT SUM(InvoiceAmount) AS Overdue FROM ml_bi_invoices WHERE Status='Draft' AND  DueDate < '$fecha 23:59:59' "; 
	...
	function getState($id)
	{
		...
		$query = $this->db->get('ml_co_country_states');
