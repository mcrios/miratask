...
class Document extends CI_Controller {
	...
	public function eraseListConfirm(){
		...
		$data['nameObject']=$this->lang->line('main_15');
	...
	public function eraseList(){
		...
		if(count($dataDelete)>0){
			$actualDir = trim(shell_exec('chdir'));
			foreach ($dataDelete as $value){
				echo $result = $this->DocumentModel->deleteDocument($value, $actualDir);
				// $result = $this->DocumentModel->deleteOne($value); //metodo que se utilizaba antes de deleteDocument
				chdir( trim($actualDir) );
			}
