...
class Ajax extends CI_Controller {
	...
	function addArea(){
		...
		$this->load->view('Matters/addArea');
	...
	function ContactNewSave(){
		...
		IF($status=="error"){
			...
			$name=$this->input->post('Name')." ".$this->input->post('Middle')." ".$this->input->post('LastName')." ".$this->input->post('Suffix');
			if ($this->input->post('Class')=="Business-Org") {
				echo "ok,".$name.",".$insert_id.",Bussiness,".$this->input->post('Company');	
			}else{
				echo "ok,".$name.",".$insert_id.",Individual, ";
			}
	...
	public function  ToGroup($oper){
		...
		if(count($GroupItem)>0 && count($contactItem)>0){
...

			{
...

				{
					...
					{
						$this->db->select();
						$this->db->from('ml_co_group_contact');
						$this->db->where('IdContact', $Contact )->where('IdGroup', $Group );

						$estadoExistencia = $this->db->get()->row();
						if(count($estadoExistencia) > 0 ){
...

		{
...

			{
				...
				if($userDB->Email!=""){
					...
					$content.="<h4>Asunto: ".$this->input->post('Subject')."</h4>";

					$content.= "<h4>".$this->session->userdata('Name').' '.$this->session->userdata('LastName').' te ha asignado una nueva tarea en '.base_url()."</h4>";

					$content.="<br><h4>Fecha de inicio: ".$starTime."</h4>";

					$content.="<br><h4>Fecha de finalización: ".$endTime."</h4>";

					$content.="<br><h4>Descripción: ".$this->input->post('Description')."</h4><br><br><br>";



					$content.='<br><h4>Para más detalles de click en el siguiente enlace: ';

					$content.='<a href="'.base_app().'Task/TaskDetail/'.$insert_id.'">'.base_app().'Task/TaskDetail/'.$insert_id.'</a></h4><br><br>';
