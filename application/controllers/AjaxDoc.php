...
class AjaxDoc extends CI_Controller {
	...
	function newDocument($IdMatter=""){
		...
		$data['Folders']=$this->DocumentModel->get_folders();;
	...
	function  upload($folder="1"){
		...
		$FisicalDir="./docsxup/".date('Y').'/';
		
		if (!is_dir($FisicalDir)) {
			mkdir($FisicalDir);

		}

		$config['upload_path']          = $FisicalDir;
		...
		if($status==''){
            //find in subfolder by years
			for($i=2018;$i<=date('Y');$i++){
				if(file_exists('./docsxup/'.$i.'/'.$NameFile)){
					$status='File already exists!'; 
					$dirFound='./docsxup/'.$i.'/';
				}
			}
		...
		if($status=='File already exists!'){ 
			...
			if (count($this->DocumentModel->SelectByName($NameFile))<1){
				
			}
		}
        //if no previous erros , proced to uplodad fisical file
		if($status==""){
		...
		if($status==""){ 
			...
			if($las_id>0){
				...
				$status='File Upload but not was registered in the System';
			}
		}
		...
		if($status!=""){
			echo '{"success":false,"error":"'.$status.'"}';
		}
