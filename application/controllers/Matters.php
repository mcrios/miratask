...
class Matters extends CI_Controller {
	...
	public function SaveNew(){
		...
		if($status==""){
			...
			if($insert_id!=""){
				...
				if ($this->input->post('TotalAmount')!='') {
				//save de billling data
					...
					foreach($time_expense as $i => $te):
						...
						$cliente = $this->billing->get_matter_name($insert_id);
				...
				if (count($fromList)>0)
				{
					foreach ($fromList AS $key => $val)
					{
						...
						if(count($total)<1){
							...
							$this->notificar_staff($val,$matter_name);
						}
	...
	function saveArea(){
		...
		$this->form_validation->set_rules('areaName', 'areaName', 'trim|required|min_length[2]');
		...
		$insert_id = $this->MatterModel->saveArea($data);

		if($insert_id <= 0){
			echo "error";
		}else{

			$newArea = $this->MatterModel->selectOneArea($insert_id);
			echo json_encode($newArea);
		}
	}
	...
	public function eraseListConfirm(){
		...
		$data['nameObject']=$this->lang->line('main_14');
		$data["ObjectNomb"]=$ItemNomb;
		$data["vista"]="global/Confirm";
			...
			if($insert_id!=""){
				...
				if (count($fromList)>0)
				{
					foreach ($fromList AS $key => $val)
					{
						...
						$total=$this->MatterModel->getOneRelatedStaff(array('IdUser'=>$val,'IdMatter'=>$Id));	
	...
	function notificar_staff($id_user,$name_matter){
		$this->load->helper('sendmail_helper');
		$this->load->helper('util_helper');
		$user = $this->MatterModel->user_get($id_user);

		$tmp_mail = $this->load->view('mails/mail-notificacion-nuevo-matter',array(), true);
		$param["name"] = $user->Name;
		$param["matter_name"] = $name_matter;
		$html = plantilla_aplicar($tmp_mail,$param);
		SendMail($user->Email,$html,"Miratask - A new matter was assigned to you");
	}
