...
class Ajax extends CI_Controller {
	...
	function ContactNew(){
		...
		$data['Countries']   = $this->UserModel->GetListFrom("ml_co_country", " WHERE Id='62' OR Id='68' OR Id='157'");
		...
		if($status=="")
		{
			...
			if($insert_id!="")
			{
				...
				for($i=1;$i<=$totAddressCont;$i++)
				{
					...
					$data = array(
						'Country'  =>$this->input->post('Country1'.$i),
		...
		if($status=="")
		{
			...
			if($insert_id!="")
			{
				...
				if($userDB->Email!=""){
					...
					$content ="<h2>Has been asigned task for you</h2>";

					$content.= $this->session->userdata('Name').' '.$this->session->userdata('LastName').' Has asigned task for you in miralawgroup.com ';
					$content.="<br>Start Day: ".$starTime;
					$content.="<br>End Day: ".$endTime;
					$content.="<br>Description: ".$this->input->post('Description')."<br><br><br>";

					$content.='<br>View More details on next Link: ';
					$content.='<a href="'.base_app().'Task/TaskDetail/'.$insert_id.'">'.base_app().'Task/TaskDetail/'.$insert_id.'</a><br><br>';

					 

					$content.='<br><img src="'.base_url().'img/img2.png">';
					$content.='<br><a href="'.base_url().'">miralawgroup.com</a>';



                	SendMail($userDB->Email,$content,"Has been asigned task for you");
	...
	function Search()
	{
		...
		foreach ($result as $row)
		{
			...
			echo '<li > 
						...
						<div class="desc"> <a '.$enlace.' >'.$row->titulo.'</a> </div> 
				</li>';
